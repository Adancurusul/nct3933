    ...
    //D: DelayMs<u8>,
{
    ...
    ///read register
    pub async fn read_register(&mut self, reg_addr: u8) -> Result<u8, NCT3933Error<E>> {
        let mut data = [0];
        self.i2c
    ...
    ///write register
    pub async fn write_register(&mut self, reg: u8, value: u8) -> Result<(), NCT3933Error<E>> {
        self.i2c
    ...
    pub async fn check_id(&mut self) -> Result<(), NCT3933Error<E>> {
        let id1 = self.read_register(ID1_ADDR).await?;
        let id2 = self.read_register(ID2_ADDR).await?;
    ...
    ///read the ID data
    pub async fn read_id_data(&mut self) -> Result<u16, NCT3933Error<E>> {
        let id1 = self.read_register(ID1_ADDR).await?;
        let id2 = self.read_register(ID2_ADDR).await?;
        Ok((id1 as u16) << 8 | id2 as u16)
    }
    ///read the WDT state
    pub async fn read_wdt_state(&mut self) -> Result<u8, NCT3933Error<E>> {
        let setting1 = self.read_register(SETTING1_ADDR).await?;
    ...
    pub async fn set_wdt_state(
        &mut self,
        enable_state: u8,
        delay: u8,
    ) -> Result<(), NCT3933Error<E>> {
        if delay > 3 || enable_state > 1 {
            return Err(NCT3933Error::InvalidChannel);
        }
        let setting1 = self.read_register(SETTING1_ADDR).await?;
        let new_setting1: u8 = (setting1 & 0x0F) | ((enable_state << 7) + (delay << 4));
        self.write_register(SETTING1_ADDR, new_setting1).await?;
        Ok(())
    }

    ///set the PS mode
    pub async fn set_ps_mode(&mut self, enable_state: u8) -> Result<(), NCT3933Error<E>> {
        if enable_state > 1 {
            return Err(NCT3933Error::InvalidMode);
        }

        let setting1 = self.read_register(SETTING2_ADDR).await?;

        let new_setting1: u8 = (setting1 & 0xBF) | (enable_state << 6); //bit 6
        self.write_register(SETTING2_ADDR, new_setting1).await?;
        Ok(())
    }
    ///set the gain
    ///channel : 1-3, mode 0/1
    pub async fn set_gain(&mut self, channel: u8, mode: u8) -> Result<(), NCT3933Error<E>> {
        if mode > 1 {
            return Err(NCT3933Error::InvalidMode);
        }
        let setting1 = self.read_register(SETTING2_ADDR).await?;
    ...
    pub async fn set_current(&mut self, channel: u8, current: i16) -> Result<(), NCT3933Error<E>> {
        if channel > 3 || channel < 1 {
            return Err(NCT3933Error::InvalidChannel);
        }
        let reg_data: u8;
        if current >= -1270 && current <= 1270 {
            self.set_gain(channel, 0).await?;
            if current <= 0 {
        ...
        self.write_register(channel, reg_data).await?;
